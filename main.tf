...
locals {
  ...
  backup_copy_actions = {
    ...
    "arn:aws:backup:${var.target_resource_region}:${var.account_id}:backup-vault:${var.vault_name}" = {
      target_backup_vault_arn = {
        "@@assign" = "arn:aws:backup:${var.target_resource_region}:${var.account_id}:backup-vault:${var.vault_name}"
      }
      lifecycle = local.backup_lifecycle
    },
  }
